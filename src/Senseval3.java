...
public class Senseval3 {
	...
	static final TokenizerFactory NGRAM_TOKENIZER_FACTORY = new NGramTokenizerFactory(
			4, 6);

	static final TokenizerFactory SPACE_TOKENIZER_FACTORY = new RegExTokenizerFactory(
			"\\S+");

	static final TokenizerFactory NORM_TOKENIZER_FACTORY = normTokenizerFactory();
	...
	private static final String DICT_FILE = "C:\\Project\\Pronto\\LingPipe\\SenseEval\\EnglishLS.train.tar\\EnglishLS.dictionary.xml";
	private static final String TRAINING_FILE = "C:\\Project\\Pronto\\LingPipe\\SenseEval\\EnglishLS.train.tar\\EnglishLS.train";
	private static final String TESTING_FILE = "C:\\Project\\Pronto\\LingPipe\\SenseEval\\EnglishLS.test.tar\\EnglishLS.test";
	private static final String SYSTEM_RESP_FILE = "C:\\Project\\Pronto\\LingPipe\\SenseEval\\systemAnswer.txt";
	...
	public static void main(String[] args) throws ClassNotFoundException,
		...
		File responseFile = new File(SYSTEM_RESP_FILE);
		sClassifierNumber = Integer.valueOf(2);

		System.out.println("Dictionary File=" + dictFile.getCanonicalPath());
		System.out.println("Training File=" + trainFile.getCanonicalPath());
		System.out.println("Testing File=" + testFile.getCanonicalPath());
		...
		System.out
				.println("     #test cases=" + testData.mWordsPlusCats.size());
	...
	static ObjectHandler<Classified<CharSequence>> createClassifierTrainer(
			String[] senseIds) {
		...
		case 1: // CONFIGURABLE CHARACTER LM CLASSIFIER
			...
			return new DynamicLMClassifier<LanguageModel.Dynamic>(senseIds,
					lms5);
		...
		case 3: // DEFAULT TOKEN UNIGRAM LM CLASSIFIER
			return DynamicLMClassifier.createTokenized(senseIds,
					NORM_TOKENIZER_FACTORY, 1);

		case 4: // DEFAULT TOKEN BIGRAM LM CLASSIFIER
			return DynamicLMClassifier.createTokenized(senseIds,
					NORM_TOKENIZER_FACTORY, 2);

		case 5: // CONFIGURABLE TOKENIZED LM CLASSIFIER W. CHARACTER BOUNDARY LM
				// SMOOTHING
			...
			for (int i = 0; i < lms2.length; ++i)
				lms2[i] = new TokenizedLM(NORM_TOKENIZER_FACTORY,
						...
						new NGramBoundaryLM(4, 128, 0.5, '\uFFFF'), 0.1); // interpolation
																			// param
			return new DynamicLMClassifier<LanguageModel.Dynamic>(senseIds,
					lms2);

		case 6: // TF-IDF CLASSIFIER
			FeatureExtractor<CharSequence> featureExtractor5 = new TokenFeatureExtractor(
		...
		case 7: // K-NEAREST NEIGHBORS DEFAULT CLASSIFIER (EUCLIDEAN DISTANCE)
			FeatureExtractor<CharSequence> featureExtractor7 = new TokenFeatureExtractor(
					SPACE_TOKENIZER_FACTORY);
			return new KnnClassifier<CharSequence>(featureExtractor7, 16); // num
																			// neighbors
																			// to
																			// average

		case 8: // K-NEAREST NEIGHBORS DEFAULT CLASSIFIER (COSINE DISTANCE)
			FeatureExtractor<CharSequence> featureExtractor8 = new TokenFeatureExtractor(
					NGRAM_TOKENIZER_FACTORY);
			return new KnnClassifier<CharSequence>(featureExtractor8, 5, // num
																			// neighbors
																			// to
																			// average
					new CosineDistance(), true);
	...
	static void respond(SenseEvalModel model, TestData testData, File file)
		...
		for (int i = 0; i < testData.mWordsPlusCats.size(); ++i) {
			...
			bufWriter.write(wordPlusCat + " " + wordPlusCat + ".bnc."
					+ instanceId);
			if (classification instanceof ConditionalClassification) {
				ConditionalClassification condClassification = (ConditionalClassification) classification;
				for (int rank = 0; rank < condClassification.size(); ++rank) {
					int conditionalProb = (int) java.lang.Math
							.round(1000.0 * condClassification
									.conditionalProbability(rank));
					if (rank > 0 && conditionalProb < 1)
						break;
	...
	static class SenseEvalModel extends
			HashMap<String, BaseClassifier<CharSequence>> {
		...
		SenseEvalModel(SenseEvalDict dict, TrainingData trainingData)
			...
			for (String wordPlusCat : trainingData.keySet()) {
				Map<String, List<String>> senseToTextList = trainingData
						.get(wordPlusCat);
				String[] senseIds = senseToTextList.keySet().<String> toArray(
						new String[0]);
				System.out.println("    " + wordPlusCat + " ["
						+ senseIds.length + " senses]");
				ObjectHandler<Classified<CharSequence>> trainer = createClassifierTrainer(senseIds);
				for (String senseId : senseToTextList.keySet()) {
					Classification classificationForSenseId = new Classification(
							senseId);
					List<String> trainingTextList = senseToTextList
							.get(senseId);
	...
	static class TrainingData extends
	...
	static class SenseEvalDict extends HashMap<String, Sense[]> {
		...
		int readDictionary(String[] lines, int pos) {
			...
			Sense[] senses = senseList.<Sense> toArray(new Sense[senseList
					.size()]);
	...
	static class Sense {
		...
		public String toString() {
			return "ID=" + mId + " SRC=" + mSource + " SYNSET=" + mSynset
					+ " GLOSS=" + mGloss;
		}
	}
